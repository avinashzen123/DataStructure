...
public class RobotIsTherePathUsingGraph {
	int V;
	List<List<Integer>> adj;

	RobotIsTherePathUsingGraph(int V) {
		this.V = V;
		adj = new ArrayList<>(V);
		for (int i = 0; i < V; i++) {
	...
	static boolean findPath(int[][] M) {
		...
		int N = M.length;
		int V = N * N + 2;
		RobotIsTherePathUsingGraph g = new RobotIsTherePathUsingGraph(V);
		...
		System.out.println("Adjacency " + g.adj.toString());
		// find path Using BFS
		return g.BFS(s, d);
	}
